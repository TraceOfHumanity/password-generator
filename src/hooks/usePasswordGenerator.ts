...
import {setIncludeNumbers, setIncludeSymbols, setIncludeUppercase, setPassword} from "@/redux/features/passwordSlice";

export const usePasswordGenerator = () => {
  const dispatch = useDispatch();
  const {passwordLength, includeUppercase, includeNumbers, includeSymbols} = useAppSelector((state) => state.password);
